...
プロダクション向けの機能として、自動再接続／キープアライブ、パス単位・リクエスト単位のメトリクス、輻輳制御とマルチパススケジューラの選択、認証前コネクション上限、マスカレードモード、INI 設定ファイル、systemd サービスパッケージングを備えています。
...
[ソースからのビルド](./building) と [クイックスタート](./quick-start) へ進むか、特定のモードへジャンプしてください: [HTTP ゲートウェイ](./http-gateway)、[UDP リレー](./udp-relay)、[透過キャプチャ](./transparent-capture)、[TLS MITM](./tls-mitm)。
